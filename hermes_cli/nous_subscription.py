...
def get_nous_subscription_features(
    ...
    video_uses_fal = video_provider in {"", "fal"}
    ...
    if web_use_gateway:
        ...
        direct_tavily = False
    # FAL is the shared upstream for both image and video gen; per-toolset
    # ``use_gateway`` flags suppress direct credentials independently so the
    # user can opt one toolset into managed mode without forcing the other
    # (e.g. direct FAL_KEY for image, managed gateway for video).
    direct_fal_for_image = False if image_use_gateway else direct_fal
    direct_fal_for_video = False if video_use_gateway or not video_uses_fal else direct_fal
    # Legacy alias retained for downstream code that still reads
    # ``direct_fal`` for image-gen flow paths.
    direct_fal = direct_fal_for_image
    if tts_use_gateway:
    ...
    managed_video_available = managed_tools_flag and nous_auth_present and is_managed_tool_gateway_ready("fal-queue")
    ...
    image_managed = image_tool_enabled and managed_image_available and not direct_fal_for_image
    image_active = bool(image_tool_enabled and (image_managed or direct_fal_for_image))
    image_available = bool(managed_image_available or direct_fal_for_image)

    video_managed = video_tool_enabled and video_uses_fal and managed_video_available and not direct_fal_for_video
    video_active = bool(video_tool_enabled and (video_managed or direct_fal_for_video))
    video_available = bool(video_uses_fal and (managed_video_available or direct_fal_for_video))
    ...
    features = {
        ...
        "image_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal_for_image else ("Nous Subscription" if image_managed else ""),
            explicit_configured=bool(direct_fal_for_image),
        ),
        "video_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal_for_video else ("Nous Subscription" if video_managed else ""),
            explicit_configured=bool(direct_fal_for_video),
        ),
